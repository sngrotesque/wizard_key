...
def attack(ciphertext):
    ...
    def split_string(x, n): return [x[i:i+n] for i in range(0, len(x), n)]
    ...
    for block_n in range(len(blocks) - 1, 0, -1):
        ...
        for byte in range(BLOCK_SIZE - 1, -1, -1):
            ...
            for i in range(0, 256):
                ...
                hacked_ciphertext = spliced_ciphertext[:byte] + attack_str + \
                ...
                if (is_padding_ok(hacked_ciphertext)):
                    ...
                    decoded_bytes = decoded_bytes[:byte] + bytearray.fromhex('{:02x}'.format(
                        hacked_ciphertext[byte] ^ new_pad_len)) + decoded_bytes[byte + 1:]
...
def test_the_attack():
    messages = (b'Attack at dawn',
                ...
                b'Oh fuck, holy shit.')
    for msg in messages:
        print(f'Testing: {msg}, OF LENGTH: {len(msg)}')
