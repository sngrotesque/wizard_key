...
4.  对于类类型的要求，构造函数尽量不要使用 [**成员初始化列表（member initializer list）**](https://en.cppreference.com/w/cpp/language/initializer_list.html)（当前对于移动构造函数等场景是可以使用的），这并不是因为它不好，而是要统一编码风格，并且有些构造函数其实只需要初始化数据成员，而空着一个`{}`很不协调。  
