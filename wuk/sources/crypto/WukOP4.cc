...
OP4_SI(void) shift_bits_add(wByte state[wuk::crypto::OP4_BL])
{
    wU32 v0, v1, v2, v3;
...
OP4_SI(void) shift_bits_sub(wByte state[wuk::crypto::OP4_BL])
{
    wU32 v0, v1, v2, v3;
...
OP4_SI(void) multiply(wByte state[wuk::crypto::OP4_BL])
{
...
OP4_SI(void) inv_multiply(wByte state[wuk::crypto::OP4_BL])
{
...
{
    for (wU32 i = 0; i < wuk::crypto::OP4_BL; ++i) {
        state[i] ^= iv[i];
    }
}
...
{
    memcpy(state, input, wuk::crypto::OP4_BL);
    for (wU32 r = wuk::crypto::OP4_NR; r-- > 0; ) {
        xor_with_iv(state, round_key + wuk::crypto::OP4_BL * r);
        inv_multiply(state);
...
namespace wuk::crypto {
    OP4::OP4(const wByte key[OP4_KL], wU32 counter)
    : counter(counter)
    {
        if (!key) {
    ...
    void OP4::ecb_encrypt(wByte *out, const wByte *in, wSize length)
    {
        if(!out || !in) {
    ...
    void OP4::ecb_decrypt(wByte *out, const wByte *in, wSize length)
    {
        if(!out || !in) {
    ...
    void OP4::cbc_encrypt(wByte *out, const wByte *in, wSize length,
    ...
    void OP4::cbc_decrypt(wByte *out, const wByte *in, wSize length,
                    const wByte iv[OP4_BL])
    {
        if(!out || !in || !iv) {
    ...
    void OP4::ofb_stream(wByte *out, const wByte *in, wSize length,
    ...
    void OP4::ctr_stream(wByte *out, const wByte *in, wSize length,
