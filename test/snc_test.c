...
void snc_speed_test()
{
    ...
    wmkcMem_new(wmkcTime_obj *, timer, sizeof(wmkcTime_obj));
    wmkcMem_new(wmkcByte *, buf, size);
...
void snc_test()
{
    ...
    for(wmkc_u32 i = 0; i < 7; ++i) {
        ...
        if(!wmkcMem_new(wmkcByte *, buf, wmkcPad_offset(SNC_BLOCKLEN, fileSize) + fileSize)) {
            printf("Memory error.\n");
            return;}
...
void test()
{
    ...
    printf("Cipher info:\n");
    printf("\tMode: SNC"WMKC_COLOR_SET(WMKC_COLOR_FORE_LIGHTRED, "%u, "), (snc->mode + 1) * 256);
    printf("\tKeySize: "WMKC_COLOR_SET(WMKC_COLOR_FORE_LIGHTCYAN, "%u, "),   snc->KN);
    printf("\tRounds:  "WMKC_COLOR_SET(WMKC_COLOR_FORE_LIGHTMAGENTA, "%u")"\n", snc->NR);
