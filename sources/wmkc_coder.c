...
WMKC_OF((wmkcChar **dst, wmkcChar *src, wmkcChar *code_string))
{
    ...
    wmkcSize dst_size = src_size * 4 / 3;
    ...
    if(!wmkcMemoryNew(wmkcChar *, (*dst), dst_size)) {
        wmkcErr_return(error, wmkcErr_ErrMemory,
            "wmkcCoder_encoding: Failed to allocate memory for (*dst).");
    }
    dst_ptr = (*dst);
    ...
    if(cd == (iconv_t)-1) {
        wmkcErr_return(error, wmkcErr_Err32, "wmkcCoder_encoding: iconv_open failed.");
    }

    wmkcSize rc = iconv(cd, &src_ptr, &src_size, &dst_ptr, &dst_size);
    if(rc == -1) {
        wmkcErr_return(error, wmkcErr_Err32, "wmkcCoder_encoding: iconv failed.");
    }
