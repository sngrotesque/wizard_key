 ...
 * @note 无
 * @param obj 指针，指向wmkcChat对象的地址
 * @param password 指针，指向用户的密码口令字符串的地址
...
WMKC_OF((wmkcChat_obj *obj, wmkcCSTR password))
{
    ...
    // 为缓冲区申请内存空间
    if(!wmkcMem_new(wmkcByte *, buf, obj->snc->KN + SNC_BLOCKLEN)) {
        wmkcErr_return(error, wmkcErr_ErrMemory,
            "wmkcChat_initCipher: Failed to allocate memory for buf.");
    }
    // 初始化缓冲区和临时盐
    wmkcMem_secure(buf, obj->snc->KN + SNC_BLOCKLEN);
 ...
 * @authors SN-Grotesque
 * @note 无
 * @param obj 指针，指向wmkcChat对象指针的地址
 * @return 返回一个wmkcErr对象，code为0代表无错误，如果为
 *         其他值，那么需检查message与code。
 */
...
WMKC_OF((wmkcChat_obj **obj))
{
    ...
    // 为wmkcChat对象申请内存空间
    if(!wmkcMem_new(wmkcChat_obj *, (*obj), sizeof(wmkcChat_obj))) {
        wmkcErr_return(error, wmkcErr_ErrMemory,
            "wmkcChat_new: Failed to allocate memory for (*obj).");
    }

    // 将wmkcChat对象的所有成员初始化
    wmkcMem_secure((*obj)->name, WMKC_CHAT_NAME_SIZE);
    wmkcMem_secure((*obj)->hash, WMKC_CHAT_HASH_SIZE);
    wmkcMem_secure((*obj)->salt, WMKC_CHAT_SALT_SIZE);
...
WMKC_OF((wmkcChat_obj *obj))
{
    ...
    wmkcMem_secure(name, WMKC_CHAT_NAME_SIZE + 1);
    wmkcMem_secure(pass, WMKC_CHAT_PASSWORD_SIZE + 1);
    ...
    // 将密码口令清零并交由操作系统释放
    wmkcMem_secure(pass, WMKC_CHAT_PASSWORD_SIZE);
...
WMKC_OF((wmkcChat_obj *obj))
{
    ...
    */
    ...
    EVP_DigestUpdate(md_ctx, obj->name, strlen((wmkcChar *)obj->name));
...
WMKC_OF((wmkcChat_obj *obj, wmkcCSTR fn))
{
    ...
    cJSON_Delete(json);
    wmkcMem_secure(jString, fileSize);
    wmkcMem_free(jString);
...
WMKC_OF((wmkcChat_obj *obj, wmkcCSTR fn))
{
    ...
    */
    ...
    if(!(json = cJSON_CreateObject())) {
        wmkcErr_return(error, wmkcErr_ErrMemory,
            "wmkcChat_saveUserInfo: Failed to allocate memory for json.");
    }
    ...
    cJSON_AddStringToObject(json, "name", (wmkcChar *)obj->name);
    cJSON_AddStringToObject(json, "hash", (wmkcChar *)hash_hex);
    cJSON_AddStringToObject(json, "salt", (wmkcChar *)salt_hex);
    ...
    cJSON_Delete(json);
    cJSON_free(jString);
    wmkcMem_secure(hash_hex, WMKC_CHAT_HASH_SIZE << 1);
    wmkcMem_secure(salt_hex, WMKC_CHAT_SALT_SIZE << 1);
    wmkcMem_free(hash_hex);
    wmkcMem_free(salt_hex);
