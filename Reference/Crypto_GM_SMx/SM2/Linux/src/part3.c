...
int dh_step2(sm2_dh_st *dh_data_A, sm2_dh_st *dh_data_B, ec_param *ecp, int order)
{
	...
	BUFFER_APPEND_BIGNUM(dh_data_B->hash_tmp_data.buffer, dh_data_B->hash_tmp_data.position, ecp->point_byte_length, point_2->y);
	BUFFER_APPEND_STRING(dh_data_B->hash_tmp_data.buffer, dh_data_B->hash_tmp_data.position, HASH_BYTE_LENGTH, hash1.hash);
...
void test_part3(char **sm2_param, int type, int point_bit_length)
{
	...
	dh_B.klen_bit = 128;
	dh_step1(
		&dh_A, (BYTE *)sm2_param_dh_d_A[ecp->type], (BYTE *)sm2_param_dh_r_A[ecp->type], ecp);
	dh_step1(
		&dh_B, (BYTE *)sm2_param_dh_d_B[ecp->type], (BYTE *)sm2_param_dh_r_B[ecp->type], ecp);
