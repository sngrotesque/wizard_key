...
// Const for AES-128
static const uint32_t r_con[10] = {
...
int keyExpansion(const uint8_t* key, uint32_t keyLen, AESKey* aesKey) {
    if (NULL==key || NULL==aesKey) {
        printf("[-] Key expansion param is NULL\n");
        return -1;
    }
    if (keyLen!=16) {
        printf("[-] Key expansion keyLen = %d, Not supported\n", keyLen);
        return -1;
    }
    uint32_t* w = aesKey->e_k;
    uint32_t* v = aesKey->d_k;
...
int addRoundKey(uint8_t(*state)[4], const uint32_t* key) {
    ...
    for (int i=0; i<4; ++i) {
        for (int j=0; j<4; ++j) {
            k[i][j] = (uint8_t)BYTE(key[j], 3 - i);  // uint32 key[4] to uint8 k[4][4] firstly
            state[i][j] ^= k[i][j];
        }
    }
    return 0;
