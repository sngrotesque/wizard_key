...
static opt_t options_common[] = {
	...
	{RSA_OPT_KEY_SCAN, 's', "scan", no_argument, "scan and display all "
		...
		KEY_DISPLAY_DEFAULT},
	{RSA_OPT_KEY_SET_DEFAULT, 'x', "default", optional_argument,
		"If " ARG" is provided, set it as the default RSA key, "
		"otherwise, set no default RSA key"},
	{RSA_OPT_PATH, 'p', "path", no_argument, "output the key search "
		"directory. the key directory can be set by the RSA_KEYPATH "
		"environment variable. if it is not set, the current working "
...
static int optlong_register_array(opt_t *ops_arr)
{
	...
	for (cur = ops_arr; ptr < max && cur->code != RSA_OPT_MAX; cur++,
		ptr++) {
...
static rsa_key_t *rsa_dirent2key(struct dirent *ent, char accept)
{
	...
	if (!strcmp(ent->d_name, RSA_KEYLINK_PREFIX ".prv") ||
		!strcmp(ent->d_name, RSA_KEYLINK_PREFIX ".pub")) {
		goto Exit;
	}
...
static rsa_key_t *rsa_key_open_dyn(char accept)
{
	...
	while (keyring) {
		...
		if (accept == RSA_KEY_TYPE_PUBLIC &&
			!strcmp(key_data, keyring->name)) {
			break;
		}
		...
		if (accept == RSA_KEY_TYPE_PRIVATE) {
			...
			/* since we're searching all keys we don't mind about
			 * ambiguity */
			keyring->is_ambiguous[idx] = 0;
			rsa_key_enclev_set(keyring->keys[idx],
				encryption_levels[0]);
			rsa_decode(&buf, &data, &keyring->keys[idx]->exp, 
				&keyring->keys[idx]->n);

			/* exhaust the list of keys sprouting form the current
			 * link in the keyring and see if any of them can
			 * correctly decrypt the key name */
			if (!memcmp((char *)buf.arr + 1,
	...
	/* there are multiple keys with the name we're looking for -
	 * ambiguous */
	if (keyring->is_ambiguous[idx]) {
		rsa_error_message(RSA_ERR_KEYMULTIENTRIES, idx ?
			"private" : "public",  rsa_highlight_str(key_data));
		goto Exit;
	}
...
static int keyname_display_single_verbose(rsa_keyring_t *kr, char *lnkname,
	int idx, int is_ambiguous)
{
	char fmt[20];
	rsa_key_t *key;
...
static void keyname_display_verbose_idx(rsa_keyring_t *keyring, int idx)
{
	...
	for (ring_ptr = keyring; ring_ptr; ring_ptr = ring_ptr->next) {
		is_ambiguous += keyname_display_single_verbose(ring_ptr, lnk,
			idx, 0);
	}
...
static int keyname_display_single(char *fmt, rsa_keyring_t *kr, char *lnkname,
	int idx, int is_keytype_other)
{
	int do_print;
...
static void rsa_delkey_links(char keytype)
{
	...
	if (keytype & RSA_KEY_TYPE_PRIVATE) {
		...
		sprintf(lnkname, "%s/%s", key_path_get(), RSA_KEYLINK_PREFIX
			".prv");
		remove(lnkname);
	}
	if (keytype & RSA_KEY_TYPE_PUBLIC) {
...
static int rsa_setkey_links(rsa_keyring_t *keyring, char keytype)
{
	...
	if (!kr) {
		rsa_error_message(RSA_ERR_KEYNOTEXIST,
